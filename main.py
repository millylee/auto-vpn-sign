    ...
    try:
        ...
        for i, account in enumerate(accounts):
            ...
            if all(field in account and account[field] for field in required_fields):
                ...
                print(f'[Config] 账号 {i+1} 缺少必要字段 (site_url, email, password)，跳过')
...
def checkin_single_account(account: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        ...
        # 签到
        checkin_response = session.post(url=checkin_url, headers=header)
...
def send_push_notification(results: List[Dict[str, Any]]):
    """发送推送通知"""
    push_plus_token = os.environ.get('PUSHPLUS_TOKEN')
    ...
    try:
        ...
        requests.post(url=push_plus_url, data=json.dumps(push_plus_data))
        print('[CheckIn] 推送成功')
    except Exception as e:
